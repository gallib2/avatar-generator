...
const Options = ({ }) => {
		...
		return sections.map((section, index) => {
			return (
				<li
					...
					<div
						className={`pieceSectionButton ${section} ${pickedSection === section ? 'pickedSection' : ''
							}`}>
						<span>{section}</span>
					</div>
				</li>
	...
	const renderSectionItems = section => {
		console.log('scetionsItems from render ', scetionsItems)
		return (
			<div>
				{scetionsItems && scetionsItems[section].map((item, idx) => (
					<button key={`${item}_${idx}`}>
						{item}
						{/* <img src={} /> */}
					</button>
				))}
	...
	// 		</div>
	// 	);
	// }, [pickedSection, renderSectionItems]);
